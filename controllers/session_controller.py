...
class SessionController:
    ...
    def get_all_session(case_id: int, user: UserModel) -> BaseResponseModel:
        ...
        with SessionLocal() as db:
            sessions = (
                db.query(CourtSession).filter(CourtSession.case_id == case_id).all()
            )
    ...
    def get_session_by_id(session_id: int, user: UserModel) -> BaseResponseModel:
        ...
        with SessionLocal() as db:
            session = (
                db.query(CourtSession).filter(CourtSession.id == session_id).first()
            )
    ...
    def create_session(
        ...
        with SessionLocal() as db:
            case = (
                db.query(Case)
                .filter(Case.id == session_data.case_id, Case.is_deleted == False)
                .first()
            )
