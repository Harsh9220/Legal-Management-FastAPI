...
class AdminController:
    def get_open_closed_cases_dashboard(user:UserModel):
    ...
    def get_paid_unpaid_amount_dashboard(user:UserModel):
        RoleHelper.require_role(["admin"],user)
        with SessionLocal() as db:
            today=date.today()
    ...
    def get_case_status_change_dashboard(user:UserModel):
        RoleHelper.require_role(["admin"],user)
        with SessionLocal() as db:
            thirty_days_ago = date.today() - timedelta(days=30)
    ...
    def get_task_dashboard(user:UserModel):
        RoleHelper.require_role(["admin"],user)
        with SessionLocal() as db:
            today=date.today()
