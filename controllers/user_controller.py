...
class UserController:
    ...
    def create_user(user_data: CreateUserRequest, current_user: UserModel) -> BaseResponseModel:
        
        if user_data.role == "admin":
            raise HTTPException(status_code=403, detail=i18n.t("translations.PERMISSION_DENIED"))

        if user_data.role not in ["lawyer", "staff", "client"]:
            raise HTTPException(status_code=400, detail=i18n.t("Invalid role specified"))
        ...
        with SessionLocal() as db:
            if db.query(User).filter(User.username == user_data.username).first():
                raise HTTPException(status_code=400, detail=i18n.t("translations.USERNAME_EXISTS"))
            
            if db.query(User).filter(User.email == user_data.email).first():
                raise HTTPException(status_code=400, detail=i18n.t("translations.EMAIL_EXISTS"))
    ...
    def get_user_by_id(user_id: int, current_user: UserModel) -> BaseResponseModel:
        ...
        with SessionLocal() as db:
            ...
            if not user:
                raise HTTPException(status_code=404, detail=i18n.t("translations.USER_NOT_FOUND"))
            ...
            if user.role == "lawyer":
                ...
                if user.id==current_user.id:
                    RoleHelper.require_role(["admin","lawyer","staff","client"],current_user)
                else:
                    RoleHelper.require_role(["admin","lawyer"], current_user)
    ...
    def update_user(user_id: int, update_data: UpdateUserRequest, current_user: UserModel) -> BaseResponseModel:
    ...
    def soft_delete_user(user_id: int, current_user: UserModel) -> BaseResponseModel:
        with SessionLocal() as db:
            ...
            if user.is_deleted:
                raise HTTPException(
                    status_code=400, 
                    detail=i18n.t("translations.USER_ALREADY_DELETED")
                )
    ...
    def restore_user(user_id: int, current_user: UserModel) -> BaseResponseModel:
        ...
        with SessionLocal() as db:
            ...
            if not user.is_deleted:
                raise HTTPException(
                    status_code=400, 
                    detail=i18n.t("translations.USER_NOT_DELETED")
                )
