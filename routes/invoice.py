...
@invoice.get("/invoices", status_code=status.HTTP_200_OK)
async def get_all_invoice(current_user: user_dependency):
    return InvoiceController.get_all_invoice(current_user)


@invoice.get("/invoices/{invoice_id}", status_code=status.HTTP_200_OK)
...
async def create_invoice(
    invoice_data: CreateInvoiceRequest, current_user: user_dependency
):
...
async def update_invoice(
    invoice_id: int, update_data: UpdateInvoiceRequest, current_user: user_dependency
):
